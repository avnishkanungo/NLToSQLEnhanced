...
def select_table(desc_path,llm):
    ...
    table_details_prompt = f"""Return the names of ALL the SQL tables that MIGHT be relevant to the user question, refer to the descriptions of the tables which also contain the information of the table columns(keys) and which key can be used to join the tables to other tables. \
            The tables are:
...
def prompt_creation(example_path):
    ...
    final_prompt = ChatPromptTemplate.from_messages(
        [
        ("system", "You are a MySQL expert. Given an input question, create a syntactically correct MySQL query to run. Unless otherwise specificed.\n\nHere is the relevant table info: {table_info}\n\nBelow are a number of examples of questions and their corresponding SQL queries. Ensure that the correct SQL syntax is used, DO NOT INCLUDE MARKDOWN IN THE SQL QUERY."),
            few_shot_prompt,
...
def initialize_chain(db_user, db_password, db_host, db_name, API_KEY, example_path=None, desc_path=None):
    ...
    if example_path is None:
        example_path = os.path.join(os.getcwd(), "NLToSQL", "few_shot_samples.json")
    if desc_path is None:
        desc_path = os.path.join(os.getcwd(), "NLToSQL", "database_table_descriptions.csv")

    final_prompt = prompt_creation(example_path)
    ...
    chain = (
        RunnablePassthrough.assign(table_names_to_use=select_table(desc_path,llm)) |
...
if __name__ == '__main__':
    ...
    parser.add_argument('--example_path', type=str, default=os.getcwd()+"/NLToSQL/few_shot_samples.json")
    parser.add_argument('--desc_path', type=str, default=os.getcwd()+"/NLToSQL/database_table_descriptions.csv")
    ...
    chain = (
        RunnablePassthrough.assign(table_names_to_use=select_table(args.desc_path,llm)) |
        RunnablePassthrough.assign(query=generate_query).assign(
        result=itemgetter("query") | execute_query
        )
        | rephrase_answer(llm)
